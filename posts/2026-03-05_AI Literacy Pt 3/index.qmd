...
Now it's time to get into the weeds a bit. I will argue that it's not enough to know some high level AI definitions and to explore and experiment with it in a systematic way. If you don't know *how* AI works, you won't use it effectively, and it limits your ability to evaluate it critically. 

A caveat: you, dear reader, may well understand the architecture of AI much better than me. In fact, one of the reasons I'm writing this post is to consolidate and crystalize what I've learned, at least as of today. Here is the other thing--in a philosophical sense, no one really knows how AI works. But that doesn't necessarily invalidate AI applications. LLMs are built using machine learning, with neural networks that attempt to mimic--at some relatively rough order of approximation--what is going on in the human brain. But what made me just wander away from the keyboard of my laptop to look out my office window? How did that trigger and influence the sentences I'm writing now? How is all this interacting with the caffeine I consumed 20 minutes ago, and the amount of REM sleep I had last night? I have no idea. And when AlphaGo made its famous "Move 37" in a 2016 match against Lee Sedol, a novel move that no human had made before, one that seemed like a mistake but was ultimately brilliant, no one could explain how that happened either.

In short, there are clearly levels to how one attempts to answer the question of how AI works. In this post I'm just going to walk us through the basic architecture and call out some useful metaphors. A big point I will make is now not novel, but worth reiterating: things are changing at such a rapid pace that it is very hard to make authoritative statements about the structural limitations of AI in performing digital tasks that involve the application of knowledge, logic and reasoning.
...
I'm not going to try to replicate the full 3.5 hours, but I will walk through the key concepts using Karpathy's framing, his clever metaphors, and some of the visuals using screenshots from his video. Then I'll point out all that has already changed in the year since he published it.
...
The terms pre-training and post-training were a little bit confusing to me, because it begs the question, "wait, when does 'training' actually happen?" I asked Claude:
...
Here's a screenshot example taken from Karpathy's video
...
- **Unverifiable domains** (creative writing, tone, summarization): quality is subjective, so it requires human judgement.
...
Sorry, Claude, that's pretty far from super funny. How was Claude trained to do this? Well, because a joke can't be scored as objectively funny or not funny, what we do instead is show humans pairs of jokes Claude has written about pelicans, and then ask which one is funnier. Do this with a thousand unique joke prompts. Use these results to have Claude update its model parameters, so that when a person requests a joke in the future, the parameters most associated with jokes ranked as more funny are weighted more heavily than those ranked as less funny. The idea here goes all the way back to psychophysics and Thurstone's work on paired comparisons. It's easier to ask people to discriminate between two jokes than it is to ask them to generate a funny joke from scratch, or to designate the criteria for what makes a joke funny. To make this scaleable, let's train a separate machine learning model on the initial sample of human pairwise preferences, so that thousands of new jokes can be generated, evaluated and used to continue updating model parameters. It's ingenious.

But there are two big problems. First, we may not be simulating human judgment very well. In theory, that seems like something we can at least check and improve. Second--and this seems like a much bigger problem--there is no guarantee that RLHF will converge to the desired solution. In verifiable domains, we have already discovered that RL can lead to outcomes that are better than what even expert humans could accomplish. Not so in RLHF. As Karpathy points out, as you continue over iterations of RLHF, you will eventually encounter adversarial solution paths. Put more simply, the model will start telling you jokes that don't even look like jokes. Because of this, when AI labs do RLHF, they have to run and monitor far fewer model iterations relative to RL with verifiable outcomes, and it is never clear whether the model has converged.  
...
But all is not lost. Because perhaps the best way to become funny is to get really good at math and coding. Yes, my psychometrician friends, there is hope for you yet :)
...
Here's what we don't know: can the changes to model parameters that happen when training these models in verifiable domains lead to the "discovery" of, what may well be even more complex, the recipe for telling a banger of a joke on demand?
...
**Simple math.** Many models claimed that 9.11 is larger than 9.9, apparently because the sequence "9:11" appears far more frequently in religious texts (Bible verse citations) than "9.9". 
...
The ability of a model to do in-context learning by holding an entire book—or an entire codebase—in its working memory is no small thing. It unlocks a qualitative change in the kinds of tasks you can reasonably expect a model to complete with accuracy. And the size of context windows has been growing over time.
...
Rather than asking a model to answer a question from its training data alone, a RAG system first retrieves relevant documents from an external corpus—a website, a company's internal knowledge base, a medical literature database, a legal document repository—and inserts them into the model's context window before generating a response. The model then reasons over documents it has never been trained on, with access to information that is current, domain-specific, and verifiable. All the major frontier models now come with RAG capability.
...
The combination of tool use and RAG has shifted what hallucination looks like in practice. The failure mode is no longer "the model confidently invents a plausible-sounding fact." It is more subtle: the model may misread a retrieved document, misinterpret an ambiguous search result, or over-rely on a tool output without recognizing that the tool itself returned something incomplete. The Swiss Cheese holes haven't disappeared, but they have gotten smaller and more irregular. In Karpathy's deep dive, it was still sometimes necessary to instruct ChatGPT 4.0 to use tools to perform an analysis like counting dots or letters. With the advent of CoT reasoning, the best AI models are now quite good at knowing when it is prudent to invoke tools as a function of the kind of prompt they have received.
...
The last item on Karpathy's checklist for the future is now all the way here—agentic tasks using computer-level control. As I described in [Part 2](../2026-02-20-26_AI%20Literacy%20Pt%202/), Claude Code is not a chatbot that helps you write code. It's an agent that reads your file system, executes commands, catches errors, and iterates—autonomously—until the task is done or it runs into something it can't resolve. It's vibe coding on steroids. Here is what Karpathy posted on the application formerly known as Twitter on February 25, 2026, just about one year since releasing his LLM deep dive video.
...
Thanks Claude, well said. In part 4 of this series I will turn to the question of how we can use AI ethically. And boy oh boy is this ever the time to be grappling with that question.
